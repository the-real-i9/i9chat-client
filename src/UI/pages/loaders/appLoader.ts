...
export default async function appLoader() {
  ...
  if (sessionUser) {
    ...
    const chatIdentToHistory = new Map<string, ChatHistoryEntryT[]>()
    ...
    await Promise.all(
      [...userChats].map(async (uc) => {
        if (uc.chat_type === "DM") {
          ...
          chatIdentToHistory.set(uc.chat_ident, dmChatHistory)
        } else {
          const groupChatHistory: ChatHistoryEntryT[] =
            (await get(`group_chat/${uc.group_info?.id}/history`)) || []

          chatIdentToHistory.set(uc.chat_ident, groupChatHistory)
        }
      })
    )

    store.dispatch(
      setChatIdentToHistoryMap(Object.fromEntries(chatIdentToHistory))
    )
